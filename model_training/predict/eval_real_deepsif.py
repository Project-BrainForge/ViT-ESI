"""
Evaluate DeepSIF on EEG-only data and export predictions.

This script is intended for "real" (or EEG-only) evaluation where you don't compute
localization metrics. It loads EEG files from a simulation folder (via the match JSON),
runs a trained DeepSIF model, and saves `all_out` to a .mat file with shape:
  (n_samples, n_times, n_sources)

Typical use case in this repo:
  - n_times = 500
  - n_sources = 994 (fsav_994)
  - n_electrodes = 75 (leadfield_75_20k.mat)
"""
...
def _load_leadfield_mat(mat_path: str) -> np.ndarray:
    ...
    for k, v in m.items():
        ...
        if isinstance(v, np.ndarray) and v.ndim == 2:
            return v
    raise KeyError(
        f"No leadfield matrix found in {mat_path}. Keys={list(m.keys())}"
    )
...
def _resolve_root_base(root_simu: Path, subject_name: str) -> Path:
    ...
    Also allow passing the subject folder directly (e.g. .../simulation/fsaverage).
    """
...
def _load_eeg_matrix(path: str, n_times: int) -> np.ndarray:
    ...
    Handles Matlab struct array format used in this repo.
    """
    eeg = load_eeg_data_from_file(path)[0][0]
    eeg = np.asarray(eeg, dtype=np.float32)
    # allow (n_electrodes, n_times) or (n_times, n_electrodes)
    if eeg.ndim == 2 and eeg.shape[0] < eeg.shape[1]:
        # assume (n_electrodes, n_times)
        pass
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to leadfield .mat (e.g. D:/fyp/stESI_pub/anatomy/leadfield_75_20k.mat)",
    )
    parser.add_argument(
        "-train_run_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional direct path to DeepSIF .pt weights (overrides -train_run_dir)",
    )
    parser.add_argument(
        "-temporal_input_size",
        type=int,
        default=500,
        help="DeepSIF temporal_input_size (hidden size) used when training",
    )
    parser.add_argument(
        "-batch_size",
        type=int,
    ...
    for start in range(0, n_samples, bs):
        ...
        with torch.no_grad():
            ...
            if not args.no_gfp_scaling:
                # unscale EEG to match training/eval scaling convention
                M_unscaled = X * torch.tensor(max_batch, device=device).view(-1, 1, 1)
                # GFP scaling returns (S,T)
                J_scaled = []
